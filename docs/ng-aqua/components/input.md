...
The `nxInput` directive is used on `<input>` and `<textarea>` elements to apply standard ng-aqua styling. It is typically used within an `nx-formfield` component for proper labels, hints, and error messages. Supports both template-driven and reactive forms, with features like auto-trimming and auto-resize for textareas.
...
import { NxFormfieldModule } from '@allianz/ng-aqua/formfield';
